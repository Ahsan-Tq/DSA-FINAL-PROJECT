...
def insert_sample_pakistani_users():
    """
    Insert sample Pakistani users with 1000 starting balance if they don't already exist.
    Simple, safe function that prevents duplicates.
    """
    ensure_auth_schema()
    
    sample_users = [
        "Ali", "Ahmed", "Hassan", "Usman", "Bilal", 
        "Hamza", "Saad", "Adeel", "Fahad", "Zain"
    ]
    
    existing_users = set(get_all_usernames())
    created = {}
    ...
    for username in sample_users:
        if username not in existing_users:
            ...
            user = create_user_with_wallet(username, password_hash, "user", 1000.0)
            user["seed_password"] = "password123"
